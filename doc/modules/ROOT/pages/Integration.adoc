...
Triggers window. Here is a listing of all the shared function, with a
...
(defn bb-prepare-for-tempo-adjustments
  "Gets the Virtual CDJ ready to tweak the tempo when Break Buddy cues
  need it to, or logs an error if BLT is not configured to use a real
  player number."
  []
