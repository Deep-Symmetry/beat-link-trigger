...
`xone-recompute-on-air` because of course changing channel assignments
can affect which channels are on the air.
...
cross-fader). All it needs to do is call `xone-recompute-on-air`.
