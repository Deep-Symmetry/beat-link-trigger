...
shortly after figuring out how to give have dramatic synchronized
lighting effects and pyro without having to follow a preset playlist:
since we can load tracks on the players, and start and stop playback,
is there any way to have Beat Link Trigger perform a programmed
automatic mix if the DJ needs to urgently leave the booth for a bit,
such as for a bathroom break? I thought thiw was something the new
Show interface could make possible, with some careful setup and an
understanding of the limitations, so I built this example for him.

NOTE: This will never replace real mixing, because there is no way to
...
the necessary shared functions built in, and some xample tracks
...
Functions and the Global Setup and Global Shutdown Expressions.

To try it out, start by saving the
link:{attachmentsdir}/XoneOnAir.bls[Xone On Air show] where you like
...
about the MIDI device not being found. Plug in the mixer, close the
show, and reopen it, and things should work better. If it still
complains, your MIDI device must be named differently than expected,
the show is looking for a port with "XONE" in the name. Figure out
what your mixer's MIDI port is called on your system, then edit line
33 of the Global Setup Expression so it is a substring that matches
only that port, and try again.
...
edited at the top of the Shared Functions, as shown below.

If you are curious how the show actually works, here is all the code!
      ...
      (seesaw/invoke-later  <2>
       (seesaw/alert
        (str "<html>The XoneOnAir show was unable to open a MIDI port matching \"" mixer-device-name
             ...
             "edit the definition of <code>mixer-device-name</code> in the Global Setup Expression."
             "<br><br>Close and reopen the show to try again.")
...
show is starting up. It tries to open the mixer's MIDI port.
...
This code runs when the show is closing, and closes the mixer's MIDI
...
the documentation is incorrect about the CC number used for the
cross-fader. It appears to actually be 4 rather than 5.
