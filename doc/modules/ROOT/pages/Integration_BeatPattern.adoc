  ...
  (when-let [selection (or (get-in (show/user-data show)  ;; <1>
    ...
    (let [device-name (.full_name selection)]  ;; <2>
      (or (get @util/opened-outputs device-name)  ;; <3>
          (try
            (let [new-output (midi/midi-out  ;; <4>
        ...
        panel   (seesaw.mig/mig-panel
                 :items [["MIDI Input:" "alignx trailing"]
                         ...
                         [(seesaw/spinner :id :feedback-off
                           ...
                           :listen [:state-changed save] :enabled? false)]])
        root (seesaw/frame :title (str "MIDI Map " (seesaw/text button))
                           :content panel
                           :on-close :nothing
                           :listen [:window-closing close])]
    (swap! (seesaw/user-data button) assoc :frame root)
...
for next time. `enable` lets us enable or disable a group of UI
elements in response to the user choosing None or some other value in
the menu:Message[] menu. And `close` is used when closing the window,
to let the show know the window is gone before actually closing it.
...
did when updating the input/output menus, we take care to include the
previously-chosen device in the menu (if there was one) even if that
