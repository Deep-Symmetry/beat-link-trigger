...
[TIP]
====
This example was rebuilt to take advantage of a new feature in
...
WARNING: The expressions used by this integration tap into the Beat Link library at a deep level, so it is not compatible with the <<SimulatingPlayback.adoc#,Shallow Playback Simulator>>. Trying to use that with this show open will result in many exceptions in the log files.
...
<1> This takes the metadata that describes a track that has been
...
<3> This is a helper function called by the trigger whenever the track
...
(doseq [entry (.entrySet (.getLoadedTracks metadata-finder))]
  (swap! globals assoc-in [:qlc-cue-times (.-player (.getKey entry))]
...
<2> As explained above, we need to use the Triggers window globals, rather than the show globals, so the raw trigger can access the information it needs.
We create and store an HTTP client, an empty cue times index, and store a reference to the cue indexer so we can unregister it when the show closes.
...
special entry in the cue index map to report when they have handled a
...
This was first used in the <<Integration_XoneOnAir.adoc#,Xone:96 Channels on Air>> integration example, and could be put to good use here to offer a user interface for configuring how to connect to QLC+.
(If you studied the first version of `find-qlc-web-socket` in the <<shared-functions,Shared Functions>>, you may have noticed it using a hardcoded address for QLC+, which didn't fit with the configuration interface described <<usage,above>>.
...
So how does it work? Let's look at the new and updated shared functions.
...
(defn build-qlc-config-ui
  ...
  (let [{:keys [qlc-host qlc-port]  ;; <1>
    ...
    (seesaw.mig/mig-panel  ;; <4>
     :background (if (prefs/dark-mode?) "#330" "#dda")  ;; <5>
     :items (concat
...
Because BLT 8 and later allow the user to change whether the user interface is in dark mode, we need to pick a background that will leave the text legible.
...
As noted above, starting with BLT version 8, the user can pick light or dark interface modes. We need a function that can react when these changes occur, to adjust the show background to keep the interface legible:

[source,clojure]
----
(defn qlc-ui-updater
  "Called when the user interface theme changes."
  [ui dark? _preferences]
  (seesaw/config! ui :background (if dark? "#330" "#dda")))
----

We will arrange for this function to be called by adding more to the Global Setup Expression, which is also where we get our interface to appear at all.
...
(let [cue-indexer (build-qlc-cue-indexer trigger-globals)
      ...
      ui-updater  (partial qlc-ui-updater ui)]  ;; <2>

  ;; [Previously discussed lines omitted…]

  (swap! globals assoc :ui-updater ui-updater)  ;; <3>
  (show/swap-user-data! show assoc :show-hue 60.0)  ;; <4>
  (show/block-tracks show ui)  ;; <5>
  (configure-qlc-ws-url show trigger-globals)  ;; <6>

  ;; Arrange for the UI to respond to changes in the UI theme
  (prefs/register-ui-change-callback ui-updater))  ;; <7>
----

<1> This calls our new shared function to build the configuration user interface.

<2> This creates a function that will be called whenever we enter or leave dark mode to update the UI background to keep the text legible.

<3> This holds on to a reference to that updater function for as long as the show window is open, to prevent it from being garbage collected.

<4> This gives the show window a distinctive yellow hue, which is also used in the background of the raw trigger it adds to the Triggers window, to visually associate it with the show.

<5> This line gets rid of the user interface elements (and menu options) that relate to managing tracks and phrase triggers in the show, and by passing in the user interface we built, arranges for it to replace the show window contents.

<6> This sets up a trigger global that holds the URL that should be used to contact QLC+, which is what the configuration interface will manage for the user.
It starts with a reasonable default, if the user has not already edited it.

<7> This arranges for BLT to call our background-color updater function whenever dark mode might have changed.

=== Global Shutdown Addition

And of course we clean up after our new elements as well.

[source,clojure]
----
(prefs/unregister-ui-change-callback (:ui-updater @globals))
----

This tells BLT it no longer needs to worry about updating the background of the show window when the show closes.
It would eventually happen anyway, because without the show globals referencing the updater function, it would get garbage collected, which would cause BLT to remove it from its list of things to call, but removing it explicitly like this is clear and immediate.
...
directions to take it, please raise them in the
https://deep-symmetry.zulipchat.com/#narrow/stream/275322-beat-link-trigger[Zulip
channel].
