...
<5> This creates the row that displays the currently-configured host
name.

<6> Similarly, a row for the current port number.

<7> And the row that holds our connection status, referencing the
...
<4> This sets up a trigger global that holds the URL that should be used to contact QLC+, which is what the configuration interface will manage for the user.
