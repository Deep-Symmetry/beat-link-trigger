...
xref:attachment$/QLC-Cues.bls[integration show], then xref:Shows.adoc#opening-shows[open] and use it as described next, and ignore the rest of the explanation of how it works until you are curious about that.
...
Make sure QLC+ is running and has its web API enabled, then use the btn:[Configure] button to try again:

image:QLC_Configure.png[QLC+ connection configuration,678,274]

If you just had to properly launch QLC+ on the same machine as BLT, clicking btn:[Configure] without changing anything should be enough to get connected.
If you are running it on another machine, enter the hostname or IP address of that machine in the text field before clicking btn:[Configure].
...
The rest of the code lives in the trigger itself, edited using the trigger's xref:Triggers.adoc#context-menu[gear or context menu]:
