...
[NOTE]
====
In case you are curious, the code that implements this Timeout show is
very short! The Global Setup expression tells BLT that the show does
not use tracks, replaces the normal content of the show window with
the big clock label, and creates a background thread to update it 30
times each second:

[source,clojure]
----
(swap! globals assoc :running true)
(let [formatter (java.text.SimpleDateFormat. "HH:mm:ss.SSS")
      timestamp (seesaw/label :text "00:00:00.0000"
                              :halign :center :valign :center
                              :font "DSEG7 Classic-40")]
  (show/block-tracks show timestamp)
  (future
    (loop []
      (seesaw/invoke-now
...
And the Global Shutdown expression just tells that background thread
to stop running:
...
Since those are the only things in the show, the file is only 542
bytes long.
====
