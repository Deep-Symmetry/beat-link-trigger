...
By clicking next to the blue diamond in the lower pane, you can expand each entry and dive down into the fields and values that make it up, which can be quite a powerful way to explore the objects.
...
Once everything is set the way you want it, click kbd:[Start] and you will be prompted to choose where to save the file.
...
When you are done recording the playlist you can either click kbd:[Stop] or close the window, and the file will be finished and closed.

[[automatically-starting]]
=== Automatically Starting the Playlist Writer

If you want to be sure that playlsits are always being written, you can use the <<Settings.adoc#playlists,Settings window>>.
...
Cursive's built-in completion can be seen in action at the bottom of the above screen shot.
See the https://cursive-ide.com/userguide/[Cursive User Guide] for more information.


=== Connecting from CIDER

Once you have the nREPL server running with the CIDER handler injected, you can tell CIDER to connect to it by typing kbd:[M-x] `cider-connect` kbd:[RET] and CIDER will prompt you for the host and port information.
