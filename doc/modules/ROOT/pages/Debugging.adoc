...
can use the <<Settings.adoc#playlists,Settings window>>.
...
You would call the function `playlist-writer/write-playlist` with three arguments, `folder` (which determines the directory in which the playlist should be written), `prefix` (which specifies the first part of the playlist's filename), and `append?` which controls what happens if the file already exists.
