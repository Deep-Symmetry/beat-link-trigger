...
Beat Link Trigger, which is why this page is being worked on. You can
also consult the https://github.com/yogthos/Selmer#selmer[Selmer]
documentation for details about how its template language differs from
Django, but that document is aimed at Clojure programmers adding
Selmer to their projects, so it will likely be hard to follow.
...
`true` or `false`). These are again useful combined with teh
...
[#tag-for]
=== for
TODO: Document the `for` tag, and possibly `sum` and `now`.
...
Specifies a block of content that can be overwritten using Selmer's
https://github.com/yogthos/Selmer#template-inheritance[template
