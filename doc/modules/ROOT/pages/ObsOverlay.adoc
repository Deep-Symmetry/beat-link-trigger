...
image:ObsOverlay.png[OBS Overlay Server Window,550,250]
...
you specify your own template file, which you can do by clicking
**Choose** on the **Template** row. It will allow you to navigate to
and select any file on your system whose name ends in `.html`, `.htm`,
or `.djhtml` (HTML, or Django HTML Template, the format supported for
inserting and formatting player information into the overlay). If your
file has a different extension you can change the **Files of Type**
dropdown from “HTML Files” to “All Files” in order to select it.

Once you have chosen a template, that is stored in your preferences,
and it will automatically be used the next time you run Beat Link
Trigger (unless the file no longer exists, in which case a warning
will be displayed when you open this window, and the default template
will be used again).
...
using a URL of the format `/public/` followed by the filename. (And,
just like the template itself, your chosen public folder is stored in
the preferences, and will be used on subsequent runs of Beat Link
Trigger until you change the chosen folder, or it can no longer be
found.)
