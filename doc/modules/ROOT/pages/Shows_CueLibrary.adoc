...
[#cue-library-button]
As soon as you have a cue in the show’s library, a *Library ▾* button appears next to the *New Cue* button in any Cue Editor windows you have open for tracks in the show (and any existing cue rows also get new link buttons just below their gear buttons, described <<linked-cues,below>>).
Clicking *Library ▾* gives you a menu of all the cues that you have added to the show’s library, and choosing one creates a new cue (in the same location that the *New Cue* button would), based on the template cue that you put in the library:
...
Then we show the window, and if we get back a truthy result from that, we know the user has not clicked the *Cancel* button.
...
If we click *Cancel* here, no cue is created, and the selection we had made in the Cues Editor window remains in place.
If we type a fixture group name, choose a level, and click *OK*, the customized cue is created, and as usual its description is selected for editing in case we want to make any further tweaks:
