...
happening right now on all the decks. It can also tell the decks to
load media available anywhere on the network (in one of the player
slots or a computer running rekordbox in export mode). As shown in the
example view below, this can be an invaluable resource on its own for
running visuals for a show, even if you don’t have any triggers set
up:
