...
NOTE: It is also not yet possible to create metadata caches (as
described in <<CachingMetadata.adoc#,Older Approaches>>) for
non-rekordbox media. Since such metadata is so much less useful, and
metadata caches are essentially deprecated anyway, this is not likely
to be implemented unless you have a really compelling use case, and
