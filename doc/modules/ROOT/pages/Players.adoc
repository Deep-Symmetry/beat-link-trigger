...
None of these situations are a problem with the CDJ-3000, however. And
on pre-Nexus players, no position tracking is possible at all.
====
...
detail section. Only loops that are stored in the track can be
displayed; loops and memory points created on-the-fly by the DJ are
not visible over the network.
