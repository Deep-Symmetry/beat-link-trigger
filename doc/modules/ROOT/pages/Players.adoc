...
NOTE: It is also not yet possible to create metadata caches (see
<<caching-metadata,below>>) for non-rekordbox media. Since such
metadata is so much less useful, and metadata caches are essentially
deprecated anyway, this is not likely to be implemented unless you
have a really compelling use case, and let us know about it.
