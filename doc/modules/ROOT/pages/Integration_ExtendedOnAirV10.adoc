...
I will instead provide a walk-through of how to use the user interface provided by the link:{attachmentsdir}/ExtendedOnAirV10.bls[show file], which you can download and open whenever you want to use it.

NOTE: Because this integration requires MIDI messages from the mixer to determine a richer notion of which channels are on the air, you will need to connect a USB cable from the mixer to the computer on which BLT is running, and turn on MIDI by activating the **On** button below the USB/MIDI ports.
