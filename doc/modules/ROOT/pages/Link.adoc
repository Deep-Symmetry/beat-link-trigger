...
As a workaround until we develop a better solution to this problem, if you encounter it you can tell the `TimeFinder` to ignore the high-precision position packets by <<Expressions.adoc#editing-expressions,adding>> this function call to your <<Expressions_TriggerGlobal.adoc#global-setup-expression,Global Setup Expression>>:
...
NOTE: This window will also open whenever you choose `Link` in a trigger’s <<Triggers.adoc#message-menu,Message Menu>>, or load a trigger that is configured that way, if Carabiner is not already connected.
