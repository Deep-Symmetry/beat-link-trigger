...
<<track-context-menu,below>>.

[[track-midi-output]]
=== MIDI Output Menu

The menu:MIDI Output[] Menu lets you choose the MIDI device to which
the Track will send messages when you configure it to send MIDI
...
<<Triggers.adoc#midi-output-menu,Triggers window>>, if a MIDI Output is
...
described in the <<Expressions_ShowTrack.adoc#,Expressions>>
...
also open that editor using the <<track-context-menu,track context menu>>.
...
described in the <<Expressions_ShowTrack.adoc#track-playing-expression,Expressions>>
section. This gives you the most flexibility because in addition to
MIDI messages, you can send arbitrary UDP packets, HTTP requests, or
...
the expression editor will be opened to let you do that. You can
also open that editor using the <<track-context-menu,track
context menu>>.
...
Show’s <<Shows.adoc#enabled-default,shared menu:Enabled Default[]
menu>> (at the top of the window) is used. This allows you to enable
and disable most or all tracks using a common set of rules, while
special tracks that need to be different can still use their own
settings. All of the options in this list (except for `Default`
itself) are available in the Enabled Default menu, and have the same
meaning there, being used by any Tracks whose own Enabled menu is set
to `Default`.
...
<<Expressions_ShowTrack.adoc#track-enabled-filter-expression,example Custom
Enabled Filter Expression>> rather than this setting.
...
the <<Expressions_ShowTrack.adoc#track-enabled-filter-expression,Expressions>>
...
described in the <<Expressions_ShowTrack.adoc#,Expressions section>>.
...
if any expression associated with the Track has a value, or if there
are any Cues in the Track.
...
particular beat ranges in the Track, as discussed in
<<Shows_Cues.adoc#,that section>>.

Once you have attached any expressions to your track, a
menu:View Expressions in Report[] option will appear below
menu:Edit Track Cues[]. Selecting it tells your system browser to
open the <<Expressions_Report.adoc#>> and scroll it to the section
that holds this track’s expressions.
...
share with each other. See <<Debugging.adoc#inspecting,Inspecting Locals
and Globals>> for more details.

As mentioned at the end of the <<Shows.adoc#adding-tracks,Adding Tracks
