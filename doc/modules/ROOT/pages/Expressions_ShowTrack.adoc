...
The track expressions can be accessed from the track’s <<Shows_Tracks.adoc#track-context-menu,context menu>>.
...
<<ShowInternals.adoc#track,knows about the track>>.
...
The basic concept of an Enabled Filter is described in the <<Expressions.adoc#editing-expressions,introduction>> to the Expressions section; tracks use them in a similar way.
...
NOTE: You can also set the menu:Enabled[] menu to Default in order to run the Show-level Default Enabled Filter as described <<Shows_Tracks.adoc#enabled-menu,above>>.
This expression probably deserves to <<Expressions_ShowGlobal.adoc#show-default-enabled-filter-expression,live at the Show level>>, and then you can tweak individual tracks that should behave differently.
The expression explanation  <<Expressions_ShowGlobal.adoc#show-enhanced-on-air-example,found there>> also walks through an earlier, simpler version of the expression, and explains why it was not quite enough.
