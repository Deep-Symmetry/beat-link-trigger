...
being played on each player which is actively playing. A map indexed
by player number, this will only have non-`nil` values when there is
phrase analysis information available for the track.
...
`:phrase-intervals`:: A map of player numbers to an index of beat
...
`:playing-phrases`:: A map of playing numbers to the currently-playing
