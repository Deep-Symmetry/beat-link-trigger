...
[[uuid]]
`:uuid`:: A random UUID that gets assigned to the show when it is opened, used to identify the private namespace that will be created and used to compile its shared functions and expressions, so they don’t collide with any in the Triggers window, or in any other show.
