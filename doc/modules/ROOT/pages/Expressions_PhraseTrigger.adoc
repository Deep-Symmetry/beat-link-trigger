...
To react to more specific regions of the phrase trigger, see the <<Expressions_ShowCue.adoc#,Cue Expressions, above>>.

The phrase trigger expressions can be accessed from the phrase trigger’s <<Shows_PhraseTriggers.adoc#context-menu,context menu>>.
In addition to the `globals` and `trigger-globals` atoms and `show` value described above, these have access to a very similar `locals` atom which can be used to share values across expressions within the phrase trigger itself (but not other phrase triggers; each gets its own `locals` map), a `phrase` value that contains everything Beat Link Trigger <<ShowInternals.adoc#phrase-contents,knows about the phrase trigger>>, and individual kinds of expressions will automatically have other values available to them which make sense in the context in which the expression is used.
...
The basic concept of an Enabled Filter is described in the <<Expressions.adoc#editing-expressions,introduction>> to the Expressions section; phrase triggers use them in a similar way.
...
`phrase-type`:: A keyword which identifies the type of phrase it has been analyzed to be, corresponding to the options you can choose in the <<Shows_PhraseTriggers.adoc#phrase-picker,phrase picker>>.
...
`track-bank`:: A keyword identifying the lighting bank the DJ assigned to the track, corresponding to the options you can choose in the <<Shows_PhraseTriggers.adoc#bank-picker,bank picker>>.
