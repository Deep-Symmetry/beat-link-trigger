...
Here's how it works.
You can start by downloading the link:{attachmentsdir}/cueMidiSender.blt[configuration file] I created for him and opening that within Beat Link Trigger.
...
These are edited using the trigger’s <<Triggers.adoc#context-menu,gear or context menu>>:
...
As before, if you have no other triggers and functions you are worried about preserving, you can start by loading a fully-implemented link:{attachmentsdir}/cueMidiSender2.blt[configuration file].
