...
WARNING: The Opus Quad does not send Channels On-Air information, so no triggers or expressions can rely on this feature. If you need something like that, look into building an integration like was done for the <<Integration_XoneOnAir#,Xone:96>>.
...
NOTE: Since rekordbox is able to respond to metadata queries, if you are playing tracks from a rekordbox collection on a laptop, even on an Opus Quad, you don’t need to worry about creating metadata archives. It is only USBs mounted directly in the Opus Quad that need this extra help.
