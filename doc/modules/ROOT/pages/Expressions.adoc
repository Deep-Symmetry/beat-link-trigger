...
<<Debugging#advanced-coding,connecting an IDE>>.
====
...
functions in a show, check out the <<Integration#break-buddy,Break
Buddy>> integration example.
...
TIP: Even though we use speed control in Resolume as a running
example, and these examples still provide a useful context for
understanding how expressions work in Beat Link Trigger, there is a
better way to synchronize Resolume with your tracks today. It now
https://resolume.com/support/en/link[supports Ableton Link], and
<<Link#overview,so does BLT>>, so you can use that for easier, even
...
This is called when the player currently being tracked reports the
start of a new beat. Continuing the example started in the
...
coming from your CDJs. Set the trigger to watch the Master Player, and
enter the following code in the Beat Expression editor:

[source,clojure,subs=attributes+]
----
(.send (:chamsys-socket @locals) (:chamsys-on @locals))  {conum-guard-clojure} <1>
(future  {conum-guard-clojure} <2>
  (Thread/sleep (long (/ 30000 effective-tempo)))  {conum-guard-clojure} <3>
  (.send (:chamsys-socket @locals) (:chamsys-off @locals)))  {conum-guard-clojure} <4>
----

<1> Immediately send the UDP packet that tells MagicQ that the remote
trigger is on.

<2> We want to later tell it that it is off, but it is
...
<3> The expression will return immediately, but in the background our
...
<4> When we wake up, halfway through the beat, we send the other UDP
...
driven at the same BPM as the master CDJ.
...
can be <<ShowInternals#show,useful>>.
...
`globals` atom and `show` value described above, these have access to
a very similar `locals` atom which can be used to share values across
expressions within the track itself (but not other tracks; each gets
its own `locals` map), a `track` value that contains everything Beat
Link Trigger <<ShowInternals#track,knows about the track>>, and
individual kinds of expressions will automatically have other values
available to them which make sense in the context in which the
expression is used.
...
have access to the same `globals` and `locals` atoms, and `show` and
`track` values that track expressions in the cue’s track do. They also
can use a `cue` value that contains everything Beat Link Trigger
<<ShowInternals#cue,knows about the cue>>, and individual kinds of
expressions will automatically have other values available to them
