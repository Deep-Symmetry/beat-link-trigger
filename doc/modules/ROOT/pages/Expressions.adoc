...
the `globals` atom and `show` value described above, these have access
...
gets its own `locals` map), a `track` value that contains everything
Beat Link Trigger <<ShowInternals.adoc#track,knows about the track>>,
