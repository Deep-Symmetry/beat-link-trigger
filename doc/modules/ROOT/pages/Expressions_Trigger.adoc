...
Each trigger has its own set of expressions which can be accessed from its xref:Triggers.adoc#context-menu[context menu].
...
This is like the Global Setup Expression described xref:Expressions_TriggerGlobal.adoc#global-setup-expression[above], but it is used to set up the `locals` atom, which is shared only with other expressions on the same trigger.
...
As described in the xref:Expressions.adoc#editing-expressions[introduction] to this section, this is used when you set a trigger’s menu:Enabled[] menu to Custom.
...
xref:Triggers.adoc#watch-menu[Watch Menu] section above.
...
With this expression in place, when the trigger is active, and you fiddle with the Pitch fader on the CDJ that is playing the track, you will see entries like this in the xref:Debugging.adoc#logs[log file]:
...
Continuing the example started in the xref:Expressions_TriggerGlobal.adoc#global-setup-expression[Global Setup Expression], here is how you could synchronize the BPM of your ChamSys MagicQ console to the beats coming from your CDJs.
