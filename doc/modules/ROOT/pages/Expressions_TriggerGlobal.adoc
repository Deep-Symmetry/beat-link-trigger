...
And, starting with BLT version 8, triggers expressions xref:Expressions_v8.adoc[now live in their own namespace], which is no longer shared with shows, nor with BLT’s expression-compilation machinery, so they can safely define other atoms and values to share, beyond the `globals` map that has always been provided.
...
Using that expression to show the Player Status window means it will always be safe to try, and--even better--the window will be reopened every time you go back online, not just when initially starting the program.footnote:[Version 8 made this even easier, thanks to the new <<Settings.adoc#player-status,Settings window>>]

Here is an even more sophisticated example that creates a https://docs.oracle.com/javase/8/docs/api/java/net/DatagramSocket.html[`DatagramSocket`] for sending a remote trigger command to the ChamSys MagicQ lighting control software using its https://secure.chamsys.co.uk/help/documentation/magicq/udp.html#_chamsys_remote_ethernet_protocol[remote ethernet protocol]:
...
The <<Expressions_ShowTrack.adoc#track-beat-expression,Beat Expression>> below shows how these globals are actually used.
...
This used to include actions such as opening the xref:Players.adoc[Player Status window], however the <<Settings.adoc#player-status,Settings window>> can now take care of this for you.
...
Similarly, if you want the xref:ObsOverlay.adoc[OBS overlay server] to automatically start once Beat Link Trigger is online, you can use the <<Settings.adoc#obs-overlays,Settings window>>.
...
<3> <<Link.adoc#passive-sync,Allows>> the DJ Link network to become Tempo Master for Ableton Link. Other choices are `:off`, <<Link.adoc#syncing-triggers,`:triggers`>>, and
