...
different from low-level “Raw” Triggers, which always send something--you
...
Once you have attached any expressions to your phrase trigger, a
menu:View Expressions in Report[] option will appear below
menu:Edit Phrase Cues[]. Selecting it tells your system browser to
open the <<Expressions_Report.adoc#>> and scroll it to the section
that holds this phrase trigger’s expressions.

Below that come the expression-specific options, allowing you to open
...
If you need to make sure a set of related cues work properly together
when a track plays, and you don’t have any actual player hardware
handy to go online with, you can <<SimulatingPlayback.adoc#,simulate
playback>>.
...
https://deep-symmetry.zulipchat.com/#narrow/stream/275322-beat-link-trigger[Deep
...
When drawing playback position markers in phrase trigger cue canvases,
rather than the red color used in track waveforms, the color of the
current phrase trigger section is used, to make it easier at a glance
to tell what part of the phrase trigger is being played. When playing
the final beat of that section, you can tell whether the position is
about to loop back to the start of the section if the playback
indicator gradually fades into transparency. Otherwise, if it is
continuing on to a new section, it will instead fade to the color of
the section that is about to begin.

== Responding to Tracks without Phrase Information

If your show is relying on phrase information, and you would like for something to happen automatically when a track that is missing phrase analysis is played, you can use a <<Shows_RawTriggers.adoc#,Raw Trigger>> to achieve that.
Set the trigger to watch the master player, set its Enabled filter to Custom, and use this as its custom Enabled Expression:
