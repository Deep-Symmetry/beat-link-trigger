...
The report is broken down into sections. At the top the expressions
which apply to the show as a whole. These are followed by expressions
at the track level for each track that has them, and then for each cue
within that track that has expressions. Then any phrase triggers with
expressions are listed, followed by any cues within the phrase
triggers which have their own expressions.

image:ExpressionsReport.png[alt=Expressions Report,width=1200,height-828]
