...
the new <<Shows#,Show Interface>> added in version 0.5 is much
...
<<Integration_MIDI_rekordbox#,rekordbox cue names>> to trigger
...
<<Expressions#,Expressions section>>), the comment takes up the entire
...
or share with each other. See <<Debugging#inspecting,Inspecting Locals
