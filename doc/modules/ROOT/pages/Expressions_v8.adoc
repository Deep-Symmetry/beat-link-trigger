...
Previously, all expression code was compiled into the Clojure namespace `beat-link-trigger.expressions`, which is the same place that some important plumbing used to compile the expressions themselves lived.
Also, since Triggers window expressions as well as the expressions for any Show that was ever opened were compiled into the same namespace, you had to be careful to name shared functions in a long-winded way that they would never overwrite each other, regardless of where they lived.

This also meant that you should not use `def` to create any of your own global variables in the Shared Functions block, and should stuff any data you might need to use in the separate `globals` atoms that BLT made available to the Triggers window and each show.

Now, Triggers window expressions get compiled into their own private namespace, `beat-link-trigger.expressions.triggers`, and each show gets a private namespace of its own with a unique name like `beat-link-trigger.expressions.SHOW__70112`. The `globals` atoms in each namespace are ordinary atoms defined in that namespace, and you can `def` your own atoms and other values in your Shared Expressions block, to make your own code more readable and more idiomatic Clojure.

NOTE: As part of this change, the integration examples have been updated, and are now simpler and easier to understand, as well as teaching more typical Clojure style. Some of them needed to be <<adapting,adapted>> in order to work in this new approach.

== The Triggers Window namespace

This is now the place where all ordinary triggers, as well as shows’ raw triggers, get compiled.
...
This will also let you see all the convenience functions and namespace aliases that are pre-installed for your convenience.
(You should still be a bit careful about replacing any of those functions, because some are used by the convenience bindings in the expression editor.)
...
Although most code should continue to work fine in this new, much safer, world, there are some things that used to be possible that are either no longer recommended (because there are now safere approaches), or that need to be tweaked slightly in order to continue to work.
