...
[horizontal]
<<startup,Startup>>::
Connecting to a network of players or working offline.
...
<<Debugging#debugging,Debugging and Sharing>>:: Tools to help you
figure out what is going on in your triggers, share them between
projects or with others, and log playlists.

<<Integration#integration-examples,Integration Examples>>::
Using Beat Link Trigger to run shows with other systems.
