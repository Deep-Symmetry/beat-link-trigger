...
image:assets/TriggersMenu.png[Triggers Menu, 392]
...
a watched player, you might find it useful even if you don't need a
custom Enabled state for the trigger, to relay ongoing state
information to other systems like Resolume. But because it is called
to decide which player to track, if your trigger is set to watch Any
Player, it will be called more times than you might expect, so there
is a better expression to use for that kind of integration:
...
described in the Watch Menu section above. players which enable the
trigger are better than ones that don't; within that group, it is
...
particular track is loaded into the CDJ. To do that we would add an
Enabled Filter expression along these lines (details will vary
depending on how you want to match the track, with several options
described <<matching-tracks,below>>):
+

```clojure
(and (= rekordbox-id 148) (= track-source-player 3))
```
+
...
image:assets/TrackDescriptionMenu.png[Track Description Menu, 678]
