...
player), you can hard-code the rekordbox ID numbers in your triggers,
or you can set up a <<caching-metadata,Metadata Cache>>.
...
full complement of CDJs when you can't be sure that at some point at
least one will not be linked to the database you need metadata for,
you can set up a metadata cache before the show, and attach that to
the slot where your DJ has inserted his media.
...
metadata for, that option can save you a lot of time. Also choose the
...
couple of seconds per track, so can be a time consuming operation for
...
image:assets/CreateCache4.png[Metadata Cache Attached, 358]
