...
The `true` on the last line is there because even when we want to use
the Enabled Filter Expression for another purpose like this, it will
_still_ control whether the trigger is enabled or not, and needs to
return a true value or it will disable the filter. (And, as it turns
out, `osc-send` returns `nil`, which is not considered a true value.)
+
