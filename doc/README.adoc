...
Notice in the figure above that, as planned, each trigger is
...
In explaining the Beat Link Trigger interface and how to extend it
with custom expressions, we showed a few examples of ways to integrate
other systems. This section builds on the concepts you've learned, to
show some useful and practical integrations with systems that we have
actually used.
...
Once you have the UDP Talk server up and working, edit your Global
Setup expression to use the broadcast address and port to define a new
function, `beyond-command`, that your other expressions will be able
to use to send PangoScript commands to it:
...
and label it something like Beyond BPM Sync in the Comment field.
Configure it to Watch the Master Player, and give it a Custom Enabled
Filter:
...
always keeping BEYOND's tempo tied to the master player.
