...
stands sends an OSC message to Resolume for every status packet, even
when the pitch is not changing. That's inefficient; it puts needless
traffic on the network, and makes Resolume waste time processing
messages that don't change anything. By adding a little more
sophistication, we can keep track of the last value we sent to
Resolume, and only send a new one when it is different. We will use a
local named `:resolume-pitch` to keep track of the last value we sent:
+
...
So once again we are only doing anything when we want to be active
(that's what the `when result` clause ensures). Within that, we
...
```clojure
(and (track-matches :symmetry-media 15)
     (>= beat-number 225))
```
...
identifying keyword, followed by the player number, slot identifier,
and rekordbox ID as the nested keys, you can simplify the lookup by
just calling `find-track` with the keyword you used to store your
track map in the globals. So we could shorten the expression above to
be:
...
image:assets/BeyondMIDIsynced.png["BEYOND synced to MIDI Clock", 162, float=right]
...
MIDI events in different ways. For mcuh more information about it, see
the BEYOND MIDI Settings manual section, accesible through
menu:Help[Documentation>Settings>MIDI settings]. In this example we
will just take a quick look at mapping the first cue cell to respond
to the Beat Link Trigger we have just created. To do that, click the
...
trigger to effect, then click the btn:[Learn 1+2] button, and cause
the trigger to activate and deactivate in Beat Link Trigger. The Learn
command tells BEYOND to watch for those two MIDI events and enter them
into the grid cells for you:
...
could use a custom Deactivation Script in Beat Link Trigger to send
another Note On message when the trigger deactivates.
